...
export const courseService = {
  ...
  // 轻量版：获取单个模块的所有课时的基本信息(不包含完整内容)
  async getModuleLessonsLight(moduleId: string): Promise<Lesson[]> {
    const { data, error } = await supabase
      .from("lessons")
      .select("id, title, type, module_id, order_index")
      .eq("module_id", moduleId)
      .is("deleted_at", null) // 只获取未删除的课时
      .order("order_index");
      
    if (error) {
      console.error(`获取模块 ${moduleId} 的课时基本信息失败:`, error);
      throw error;
    }
    
    // 为每个课时添加空的content对象
    const lightLessons = data 
      ? data.map(lesson => ({
          ...lesson,
          content: {}
    ...
    return lightLessons as Lesson[];
  },
  
  // 轻量版：批量获取模块的课时基本信息
  async getModuleLessonsBatchLight(moduleIds: string[]): Promise<Record<string, Lesson[]>> {
    ...
    if (error) {
      console.error(`批量获取模块课时基本信息失败:`, error);
      throw error;
    }
    ...
    return lessonsByModule as Record<string, Lesson[]>;
  },
  
  // 获取课程轻量预览（包括模块和课时的基本信息，不包含课时详情）
  async getCourseLightPreview(courseId: string): Promise<Course & { modules?: CourseModule[] }> {
    try {
      console.time('getCourseLightPreview'); // 性能计时开始
      console.log(`开始获取课程轻量预览: ${courseId}`);
      ...
      if (!modulesData || modulesData.length === 0) {
        console.timeEnd('getCourseLightPreview');
      ...
      console.log(`批量获取 ${moduleIds.length} 个模块的课时基本信息`);
      
      const allLessonsByModuleId = await this.getModuleLessonsBatchLight(moduleIds);
      ...
      console.timeEnd('getCourseLightPreview'); // 性能计时结束
      console.log(`课程轻量预览获取完成，包含 ${modulesWithLessons.length} 个模块`);
      ...
      console.error('获取课程轻量预览时出错:', error);
      console.timeEnd('getCourseLightPreview'); // 确保在发生异常时也能结束计时器
      throw error;
    }
  },
