...
export const courseService = {
  ...
  // 标记课时为已完成
  async markLessonComplete(userId: string, courseId: string, lessonId: string, enrollmentId: string): Promise<number> {
    try {
      // 首先检查是否已经存在这个课时的完成记录
      ...
      if (checkError && checkError.code !== 'PGRST116') {
        console.error('检查课时完成记录失败:', checkError);
        throw checkError;
      }
      ...
      if (completedError) {
        console.error('获取已完成课时失败:', completedError);
      ...
      if (updateError) {
        console.error('更新课程进度失败:', updateError);
        throw updateError;
      }
